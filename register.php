...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Terms checkbox must be checked (name="terms" added to form)
    ...
    if (!$email || !filter_var($email, FILTER_VALIDATE_EMAIL)) {
        ...
        $error_message = 'Please provide your first and last name.';
    } elseif (!$accepted_terms) {
        $error_message = 'You must accept the Terms of Service to register.';
    } else {
            ...
            if (!$stmt) {
                ...
                if ($stmt->execute()) {
                    if ($role === 'veterinarian') {
                        ...
                        $app = $conn->prepare("INSERT INTO veterinarian_applications (user_id, qualifications, status) VALUES (?, ?, 'pending')");
                        $qualifications = "Pending submission"; // Placeholder
                        if ($app) {
                            $app->bind_param("is", $user_id, $qualifications);
                            $app->execute();
                            $app->close();
                        }
                    }
