...
-- Description: Comprehensive pet health management system with appointment scheduling and history
-- 
-- Key Updates:
-- - Added 'name' field to users table for backward compatibility
-- - Added 'bio', 'notification_preferences', 'privacy_settings' to users table
-- - Added 'deletion_requested', 'deletion_reason', 'deletion_requested_at' to users table
-- - Added 'notes' field to pets table
-- - Added 'date', 'heart_rate', 'notes' fields to health_records table
-- - Added 'duration' field to appointments table for compatibility
-- - Added 'notes' field to vaccinations table
-- - Updated sample data to include all new fields
...
CREATE TABLE users (
    ...
    name VARCHAR(255), -- Full name (legacy field for compatibility)
...
CREATE TABLE health_records (
    ...
    date DATE, -- Alternative field name for compatibility
...
CREATE TABLE vaccinations (
    ...
    reaction TEXT,
    notes TEXT, -- Additional notes about the vaccination
    cost DECIMAL(10,2),
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    FOREIGN KEY (pet_id) REFERENCES pets(id) ON DELETE CASCADE,
    FOREIGN KEY (veterinarian_id) REFERENCES users(id) ON DELETE SET NULL
...
CREATE TABLE appointments (
    ...
    duration INT DEFAULT 30, -- Alternative field name for compatibility
...
INSERT INTO health_records (pet_id, veterinarian_id, record_type, title, description, diagnosis, treatment, record_date, date, weight_at_visit, temperature, notes, cost) VALUES
(1, 2, 'checkup', 'Annual Wellness Exam', 'Routine annual checkup', 'Healthy', 'Continue current diet and exercise routine', '2024-05-15', '2024-05-15', 28.5, 38.5, 'Very cooperative during exam', 85.00),
(1, 2, 'vaccination', 'DHPP Vaccination', 'Annual DHPP booster vaccination', NULL, 'DHPP vaccine administered', '2024-05-15', '2024-05-15', 28.5, 38.5, 'No adverse reactions observed', 45.00),
(2, 3, 'checkup', 'Senior Cat Checkup', 'Routine senior cat wellness exam', 'Mild dental tartar', 'Dental cleaning recommended', '2024-06-20', '2024-06-20', 4.2, 38.8, 'Patient was slightly stressed but cooperative', 75.00),
(3, 2, 'illness', 'Skin Allergy Flare-up', 'Presenting with itching and redness', 'Allergic dermatitis', 'Antihistamine prescribed, hypoallergenic diet recommended', '2024-07-10', '2024-07-10', 35.8, 39.2, 'Owner advised to monitor symptoms closely', 120.00),
(4, 3, 'checkup', 'Kitten Wellness Check', 'First wellness exam for new kitten', 'Healthy kitten', 'Continue kitten food, schedule spay surgery', '2024-04-12', '2024-04-12', 3.8, 38.6, 'Very active and healthy kitten', 65.00);
